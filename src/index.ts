...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let colorIndex = 0;
    const logo = {
        width: window.innerWidth * 0.123,
        height: (window.innerWidth * 0.123) / 2,
        image: new Image(),
    }
    const position = {
        x: (window.innerWidth / 2) - (logo.width / 2),
        y: (window.innerHeight / 2) - (logo.height / 2)
    }
    const speed = {
        x: window.innerWidth * 0.001,
        y: window.innerHeight * 0.00175
    };

    ctx.canvas.width = window.innerWidth;
    ctx.canvas.height = window.innerHeight;
    logo.image.src = generateSVGSource(createDVDLogo(colors[colorIndex]));
    ...
    setInterval(() => {
        ...
        ctx.drawImage(logo.image, position.x, position.y, logo.width, logo.height);

        if (position.x < 0 || position.x > (ctx.canvas.width - logo.width)) {
            speed.x = -speed.x;
            colorIndex = Math.floor(Math.random() * colors.length);
            logo.image.src = generateSVGSource(createDVDLogo(colors[colorIndex]));
        }
        if (position.y < 0 || position.y > (ctx.canvas.height - logo.height)) {
            speed.y = -speed.y;
            colorIndex = Math.floor(Math.random() * colors.length);
            logo.image.src = generateSVGSource(createDVDLogo(colors[colorIndex]));
        }
    ...
    window.addEventListener("resize", function () {
        ctx.canvas.width = window.innerWidth;
        ctx.canvas.height = window.innerHeight;
        position.x = (ctx.canvas.width / 2) - (logo.width / 2);
        position.y = (ctx.canvas.height / 2) - (logo.height / 2);
        logo.width = window.innerWidth * 0.123;
        logo.height = logo.width / 2;
        speed.x = window.innerWidth * 0.001;
        speed.y = window.innerHeight * 0.00175;
    });
});
